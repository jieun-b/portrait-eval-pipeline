...
class Image2ImagePipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # LIA 
        ref_LIA_input = torch.cat([self.vae_image_processor.preprocess(img, height=256, width=256) for img in ref_image], dim=0)  # (B, C, H, W)
        gt_LIA_input = torch.cat([self.vae_image_processor.preprocess(img, height=256, width=256) for img in gt_image], dim=0)

        motion_embeds = self.lia(ref_LIA_input.to(device=device, dtype=self.lia.dtype), gt_LIA_input.to(device=device, dtype=self.lia.dtype))
