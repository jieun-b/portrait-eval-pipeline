...
class LivePortraitPipeline(object):
    ...
    def execute(self, args: ArgumentConfig):
        ...
        # elif osp.exists(args.driving):
            ...
            if isinstance(args.driving, list) and all(isinstance(p, str) for p in args.driving):
                flag_is_driving_video = True
                output_fps = 8
                driving_rgb_lst = [load_image_rgb(p) for p in args.driving]
                log(f"Load driving from frame list, #frames = {len(driving_rgb_lst)}")
            else:
